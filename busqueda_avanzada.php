...
$recursos = optional_param('recursos', '', PARAM_TEXT);
$ano=optional_param('ano', '', PARAM_TEXT);
...
$ordenar = optional_param('buscarselect', 'titulo', PARAM_TEXT);
...
$sqlCount = 'SELECT count(*) as  cantidad
        ...
        WHERE r.titulo like "' . $nombre . '%"
         and r.autor like "' . $autor . '%" and r.materia like "' . $materia . '%" and r.recurso like "'.$recursos.'%"  and r.ano like "'.$ano.'%" and r.palabraclave like "' . $palabraclave . '%"'. $cats .'
        order by r.id';
...
$sql = 'SELECT * FROM mdl_biblioteca_recursos'
        ...
        and palabraclave like "%' . $palabraclave . '%"
        and recurso like "%' . $recursos . '%"
        and ano like "%' . $ano . '%"
...
<div class="contenedor">
    ...
    <p><?php echo get_string("intro_1".$catr->id, "block_biblioteca")?></p>
    ...
    <form id="form1" name="form1" method="post" action="#">
      <table width="90%" border="0" align="center" cellspacing="1">
        ...
        <tr>
              ...
              <td><input name="palabraclave" type="text" id="palabraclave" size="70" /></td>
        </tr>
        <tr>
              <td height="30"><b>Tema</b></td>
              <td><input name="recursos" type="text" id="recursos" size="70" /></td>
        </tr>
        <tr>
              <td height="30"><b>Año</b></td>
              <td><input name="ano" type="text" id="ano" size="70" /></td>
        </tr>
        <tr>
  ...
  <option value="palabraclave">Palabras Claves</option>
...
<p>
    ...
    <div id="pager">
        ...
        for ($i = 0; $i < $maxpage; $i++) {
            ...
            <?php } else { ?>
      <a href="?cat=<?php echo $cat?>&page=<?php echo $i; ?>&nombre=<?php echo $nombre;?>&ano=<?php echo $ano;?>&buscarselect=<?php echo $ordenar;?>" title="Page <?php echo $i; ?>" class=""><?php echo $i; ?></a>
                <?php
            }
        }
        if ($page >= 0 && $page + 1 < $maxpage) {
            echo '<a href="?cat='.$cat.'&page=' . ($page + 1) .'&nombre='.$nombre. '&ano='.$ano. '" title="Next Page" class="rax-active-pal">' .
            '<span class="ui-icon ui-icon-triangle-1-e">>></span></a>';
        }
        if ($page + 1 == $maxpage) {
            echo '<a href="?cat='.$cat.'&page=' . ($page - 1) .'&nombre='.$nombre. '&ano='.$ano.'" title="Prvious Page" class="rax-active-pal">' .
